...
//	세션(session) : 사전적의미로는 시간을 의미함
//	클라이언트와 서버간의 통신 연결에서 두 개체의 활성화된 접속 정보를 뜻함
//	클라이언트가 서버에 요청하여 처음 서버에 접속하면 서버는 요청한 클라이언트에 대하여 고유한 임시 ID를 부여하게 되는데 이 ID를 세션이라함
//	세션 ID는 서버에 임시로 저장되며 각 페이지 이동 시 이용하거나, 클라이언트가 서버에 재접속 할경우 해당 클라이언트를 구분하는 유일한 수단임
...
//		HttpSevletRequest : 서버로 요청된 데이터에 대한 모든 내용을 가지고 있는 클래스
//		getSession() : HttpSevletRequest가 가지고 있는 클라이언트의 세션 정보를 가져옴
...
//		클라이언트의 정보를 가지고 있는 HttpServletRequest 클래스의 객체 request가져옴
//		request를 통해서 서버에 접속한 클라이언트의 세션 정보를 받아옴
//		기존에 생성된 세션 ID와 현재 logout 페이지에 접속한 클라이언트의 세션 ID가 동일함
		HttpSession session = request.getSession();
//		removeAttribute() 메서드를 사용하여 지정한 세션 정보를 삭제함
		session.removeAttribute("userId");
//		해당 세션 ID가 가지고 있는 모든 세션 정보를 삭제함
//		maxInactiveInterval의 시간이 남아 있어도 세션 정보가 삭제되었기 때문에 세션 정보를 확인할 수 있음
		session.invalidate();
