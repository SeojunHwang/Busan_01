...
//		사용자 입력을 위한 Scanner 클래스
		...
		File file = null; // 파일의 기본정보
		FileInputStream fis = null; // 파일의 내용 불러오기
		FileOutputStream fos = null; // 파일에 저장 하기
...
//			2번째 매개변수를 true로 사용 시 기존 파일에 데이터 추가하기
			...
			while (true) {
				...
				System.out.println("파일에 쓴 내용 : " + msg);
			}
		}
		catch (IOException e) {
...
//			불러올 파일의 정보
			file = new File(filePath + fileName);
			fis = new FileInputStream(file); // 내용을 확인할 파일 지정
			
			int buffSize = fis.available(); // 한번에 읽어올 byte 수 확인
			byte[] buff = new byte[buffSize]; // 필요한 byte[] 생성
			fis.read(buff); // 지정한 byte[] 에 데이터 읽어오기
			
			// 읽어온 byte데이터를 문자열로 변환하여 출력
