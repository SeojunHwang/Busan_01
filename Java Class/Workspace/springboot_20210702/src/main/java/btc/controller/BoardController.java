...
//	JPA(Java persistence API) : 자바 객체와 데이터베이스 테이블 간의 매핑을 처리하는 ORM 기술 표준
//	ORM은 객체와 관계를 설정하는 것을 뜻함, 여기서 객체는 OOP의 객체(객체 지향 언어의 객체)이고, 관계란 관계형 데이터 베이스를 의미함
//	ORM은 객체와 데이터베이스의 매핑을 도와주는 기술을 의미함
//	ORM은 특정한 언어에 종속적인 개념이 아니라 객체와 관계형 데이터 베이스를 매핑시킨다는 개념이고, 이러한 ORM의 개념을 구현하기 위한 표준이 JPA임
...
//	데이터 베이스에 독립적인 개발이 가능 : JPA가 해당 데이터베이스에 맞는 쿼리를 생성해 줌
//	유지보수가 쉬움 : JPA를 사용 시 JPA 엔티티만 수정하면 됨
	
//	JPA 단점
//	학습이 어려움 : 기존 방식에 비해서 배워야할 것이 많음, SQL 직접적으로 사용하지 않기 때문에 DB 튜닝이 어려움
...
//		insert와 update 시 모두 saveBoard() 메서드를 실행함
//		jpa의 save() 메서드는 insert 기능과 update 기능을 동시에 수행함
